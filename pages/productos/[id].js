...
const Producto = () => {
  ...
  // context de DataBase
  const { DataBase, usuario } = useContext(FirebaseContext);

  useEffect(() => {
    if (id && consultarDB) {
      const obtenerProducto = async () => {
        const productoQuery = await DataBase.db.collection("productos").doc(id);
  ...
  const eliminarProducto = async () => {
    ...
    try {
      await DataBase.db.collection("productos").doc(id).delete();
      router.push("/");
    } catch (error) {
      console.log(error);
    }
